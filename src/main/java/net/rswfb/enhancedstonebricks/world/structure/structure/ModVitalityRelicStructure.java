...
import net.rswfb.enhancedstonebricks.world.structure.piece.ModVitalityRelicStructurePieces;

import java.util.Optional;

public class ModVitalityRelicStructure extends Structure {

    public static final Codec<ModVitalityRelicStructure> CODEC = simpleCodec(ModVitalityRelicStructure::new);

    public ModVitalityRelicStructure(StructureSettings pSettings) {
        super(pSettings);
    }
    ...
    private void generatePieces(StructurePiecesBuilder pBuilder, Structure.GenerationContext pContext) {
        ...
        ModVitalityRelicStructurePieces.addPieces(pContext.structureTemplateManager(), blockpos, rotation, pBuilder, worldgenrandom);
    }

    @Override
    public StructureType<?> type() {
        return ModStructureType.MOD_VITALITY_RELIC_STRUCTURE.get();
    }
}